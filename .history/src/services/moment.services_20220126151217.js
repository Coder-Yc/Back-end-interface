...
class momnetservices {
  ...
  async getContentList(offset, size) {
    const statement =
      "SELECT m.id id, m.content content, m.createAt createTime, m.updateAt updateTime, JSON_OBJECT('id', u.id, 'name', u.name) user,(SELECT COUNT(*) FROM `comment` c WHERE c.moment_id = m.id) commentCountFROM `moment` m LEFT JOIN `users` u ON m.user_id = u.id LIMIT 0,10";
