...
const verifyPermission = async (ctx, next) => {
  const ResouseID = ctx.params;
  const userId = ctx.body.id;
  const key = Object.keys(ResouseID);
  const NeedID = ResouseID[key[0]];
  const NeedName = key[0].replace("ID", "");
  console.log(NeedID, NeedName);
  const result = await Authservice.checkMomment(NeedID, NeedName);
  // console.log(result);
